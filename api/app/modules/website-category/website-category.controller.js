...
const getListWebsiteCategory = async (req, res, next) => {
  try {
    const serviceRes = await websiteCategoryService.getListWebsiteCategory(
      req.query
    );
...
const getListAllWebsiteCategory = async (req, res, next) => {
  try {
    const serviceRes = await websiteCategoryService.getListAllWebsiteCategory(
      req.query
    );
...
const getListAllWebsite = async (req, res, next) => {
  try {
    const serviceRes = await websiteCategoryService.getListAllWebsite(
      req.query
    );
    const { data } = serviceRes.getData();
    return res.json(new SingleResponse(data));
...
const detailWebsiteCategory = async (req, res, next) => {
  try {
    const serviceRes = await websiteCategoryService.detailWebsiteCategory(
      req.params.websiteCategoryId
    );
...
const detailWebsite = async (req, res, next) => {
  try {
    const serviceRes = await websiteCategoryService.detailWebsite(
...
const createWebsiteCategory = async (req, res, next) => {
  try {
    ...
    const serviceRes =
      await websiteCategoryService.createWebsiteCategoryOrUpdate(req.body);
    if (serviceRes.isFailed()) {
      return next(serviceRes);
    }
    return res.json(
      new SingleResponse(
        serviceRes.getData(),
        RESPONSE_MSG.WEBSITECATEGORY.CREATE_SUCCESS
      )
    );
  } catch (error) {
    return next(error);
...
const updateWebsiteCategory = async (req, res, next) => {
  try {
    ...
    const serviceResDetail = await websiteCategoryService.detailWebsiteCategory(
      websiteCategoryId
    );
    ...
    return res.json(
      new SingleResponse(
        serviceRes.getData(),
        RESPONSE_MSG.WEBSITECATEGORY.UPDATE_SUCCESS
      )
    );
  } catch (error) {
    return next(error);
...
const changeStatusWebsiteCategory = async (req, res, next) => {
  try {
    ...
    // Update status
    await websiteCategoryService.changeStatusWebsiteCategory(
      websiteCategoryId,
      req.body
    );
    return res.json(
      new SingleResponse(
        null,
        RESPONSE_MSG.WEBSITECATEGORY.CHANGE_STATUS_SUCCESS
      )
    );
  } catch (error) {
    return next(error);
...
const checkParent = async (req, res, next) => {
  try {
    ...
    const serviceRes = await websiteCategoryService.checkParent(
      websiteCategoryId,
      req.body
    );
    if (serviceRes.isFailed()) {
      return res.json(
        new SingleResponse(0, RESPONSE_MSG.WEBSITECATEGORY.EXISTS_PARENT)
      );
    }
...
const deleteWebsiteCategory = async (req, res, next) => {
  try {
    ...
    const serviceRes = await websiteCategoryService.deleteWebsiteCategory(
      websiteCategoryId,
      req.body
    );
    if (serviceRes.isFailed()) {
    ...
    return res.json(
      new SingleResponse(null, RESPONSE_MSG.WEBSITECATEGORY.DELETE_SUCCESS)
    );
  } catch (error) {
    return next(error);
  }
