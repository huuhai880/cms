...
const ruleApprovedTimes = Joi.array()
  .items(
    Joi.object().keys({
      time_keeping_id: Joi.required(),
      start_time: Joi.string().regex(/^[0-9]{1,2}:[0-9]{1,2}/),
      end_time: Joi.string().regex(/^[0-9]{1,2}:[0-9]{1,2}/),
    }),
  )
  .required();
