...
const addIntergret = async (body = {}) => {
    ...
    try {
        let is_interpret_special = apiHelper.getValueFromObject(body, 'is_interpretspectial', false);
        let attribute_list = apiHelper.getValueFromObject(body, 'attribute_list', []);

        await transaction.begin();

        const reqInterpret = new sql.Request(transaction);
        const rsIntergret = await reqInterpret
            .input('INTERPRETID', apiHelper.getValueFromObject(body, 'interpret_id'))
        ...
        if (interpret_id && is_interpret_special) {
            const reqDelAttr = new sql.Request(transaction);
            await reqDelAttr
                .input('INTERPRETID', interpret_id)
        ...
        if (interpret_id && is_interpret_special) {
            if (attribute_list && attribute_list.length > 0) {
                const reqInterpretAttibutes = new sql.Request(transaction);
                for (let index = 0; index < attribute_list.length; index++) {
                    const element = attribute_list[index];
                    await reqInterpretAttibutes
                        .input('INTERPRETATTRIBUTEID', apiHelper.getValueFromObject(element, 'interpret_attribute_id'))
                        .input('ATTRIBUTEID', apiHelper.getValueFromObject(element, 'attribute_id'))
                        .input('MAINNUMBERID', apiHelper.getValueFromObject(element, 'mainnumber_id'))
                        .input('MAINNUMBER', apiHelper.getValueFromObject(element, 'mainnumber'))
                        .input('INTERPRETID', interpret_id)
                        .input('CREATEDUSER', apiHelper.getValueFromObject(body, 'auth_name'))
