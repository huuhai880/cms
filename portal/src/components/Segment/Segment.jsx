...
class Segment extends Component {
  ...
  async _getBundleData() {
    ...
    let all = [
      // @TODO:
      this._SegmentModel.getList(this.state.query)
  ...
  getData = ( query = {} ) => {
    this.setState({ isLoading: true });
    return this._SegmentModel.getList(query)
    .then(res => {
      let data = res.items;
      let isLoading = false;
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._SegmentModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleActionItemClick(type, id, rowIndex) {
    let routes = {
      ...
      edit: '/segment/edit/',
      changePassword: '/segment/change-password/',
    }
  ...
  handleClose(confirm, id, rowIndex) {
    ...
    if (confirm) {
      ...
      .catch((err) => {
        ...
        if(err.errors === null && err.message === "Segment used by SEG_DATALEADS"){
  ...
  handleSubmitFilter = (search, is_active, is_delete, create_date_from, create_date_to, company_id, business_id) => {
    let query = {...this.state.query}
    query.page = 1;
    query = Object.assign(query, {search, is_active, is_delete, create_date_from, create_date_to, company_id, business_id});
    this.getData(query)
    .catch(() => {
      window._$g.dialogs.alert(
  ...
  render() {
    const columns = [
      configIDRowTable("segment_id", "/segment/detail/", this.state.query),
      {
        name: "segment_name",
        label: "Tên phân khúc",
        options: {
                ...
                <CheckAccess permission="CRM_SEGMENT_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].segment_id, tableMeta['rowIndex'])
                    }}
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="text-center">
                <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].segment_id, tableMeta['rowIndex'])}>
                  <i className="fa fa-info" />
                </Button>
                <CheckAccess permission="CRM_SEGMENT_EDIT">
                  <Button color="primary" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].segment_id, tableMeta['rowIndex'])}
                    disabled={!(userAuth._isAdministrator() || this.state.data[tableMeta['rowIndex']].is_system === 0)}
                  >
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="CRM_SEGMENT_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].segment_id, tableMeta['rowIndex'])}
                    disabled={!(userAuth._isAdministrator() || this.state.data[tableMeta['rowIndex']].is_system === 0)}
                  >
    ...
    return (
      <div>
        ...
        <div>
          <CheckAccess permission="CRM_SEGMENT_ADD">
