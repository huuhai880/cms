...
class CampaignStatus extends Component {
   ...
   * @var {CampaignStatus}
   */
  _campaignStatusModel

  constructor(props) {
    super(props)

    // Init model(s)
    this._campaignStatusModel = window._campaignStatusModel = new CampaignStatusModel()
    // Bind method(s)
  }
  ...
  async _getBundleData() {
    ...
    let all = [
      // @TODO:
      this._campaignStatusModel.getList(this.state.query)
  ...
  getData = ( query = {} ) => {
    this.setState({ isLoading: true });
    return this._campaignStatusModel.getList(query)
    .then(res => {
      let data = res.items;
      let isLoading = false;
  ...
  handleClickAdd = () => {
    window._$g.rdr('/campaign-status/add')
  }
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._campaignStatusModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleActionItemClick(type, id, rowIndex) {
    let routes = {
      detail: '/campaign-status/detail/',
      delete: '/campaign-status/delete/',
      edit: '/campaign-status/edit/',
      changePassword: '/campaign-status/change-password/',
    }
  ...
  handleClose(confirm, id, rowIndex) {
    ...
    if (confirm) {
      this._campaignStatusModel.delete(id)
      .then(() => {
  ...
  render() {
    const columns = [
      configIDRowTable("campaign_status_id", "/campaign-status/detail/", this.state.query),
      {
                ...
                <CheckAccess permission="CRM_CAMPAIGNSTATUS_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].campaign_status_id, tableMeta['rowIndex'])
                    }}
                  />
                </CheckAccess>
              </div>
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="text-center">
                <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].campaign_status_id, tableMeta['rowIndex'])}>
                  <i className="fa fa-info" />
                </Button>
                <CheckAccess permission="CRM_CAMPAIGNSTATUS_EDIT">
                  <Button color="primary" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].campaign_status_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="CRM_CAMPAIGNSTATUS_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].campaign_status_id, tableMeta['rowIndex'])}>
    ...
    return (
      <div>
        ...
        <div>
          <CheckAccess permission="CRM_CAMPAIGNSTATUS_ADD">
          ...
          <CheckAccess permission="CRM_CAMPAIGNSTATUS_EXPORT">
