...
function Farmous() {
  ...
  const [dataFarmous, setDataFarmous] = useState([]);
  ...
  const _callAPI = async (props) => {
    try {
      await _farmousModel.getListFarmous(props).then((data) => {
        setDataFarmous(data);
      });
    } catch (error) {
      console.log(error);
  ...
  const handleDelete = (id) => {
    window._$g.dialogs.prompt("Bạn có chắc chắn muốn xóa dữ liệu đang chọn?", "xóa", (confirm) => {
      if (confirm) {
        try {
          _farmousModel.delete(id).then((data) => {
          ...
          <Col xs={12} style={{ padding: 0 }}>
            <div className="MuiPaper-root__custom">
              {isLoading ? (
                ...
                <div>
                  <MUIDataTable
                    data={dataFarmous.items}
                    columns={getColumTable(
                      dataFarmous.items,
                      dataFarmous.totalItems,
                      query,
                      handleDelete
                      // handleReply,
                      // handleReview
                    )}
                    options={configTableOptions(dataFarmous.totalItems, query.page, {
                      itemsPerPage: query.itemsPerPage,
                    })}
                  />
                  <CustomPagination
                    count={dataFarmous.totalItems}
