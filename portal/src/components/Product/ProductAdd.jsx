...
function ProductAdd({ noEdit = false, productId = null }) {
  ...
  const initData = async () => {
    ...
    try {
      if (productId) {
        ...
        setSaveDataProductPage(
          JSON.parse(JSON.stringify(product.product_page))
        );
  ...
  const callAPIInterPret = async (query) => {
    ...
    attrProduct[query.index].interprets = attrProduct[
      query.index
  ...
  const optionPageProductGroup = () => {
    ...
    if (dataProductPage && dataProductPage.length > 0) {
      return dataProductPage.map(
        ({ product_page_id: value, page_name: label }) => {
  ...
  const optionAttProductPage = (parent_key) => {
    ...
    if (productPageAttributeGroup && productPageAttributeGroup.length > 0) {
      return productPageAttributeGroup.map(
        ({ attributes_group_id: value, attributes_group_name: label }) => {
          return product_page.data_child.find(
            (p) => p.attributes_group_id == value
          )
            ? {
                value,
                label,
  ...
  const handleChangeAttributesPageProduct = (
    selected,
    record,
    index,
    parent_key
    ...
    setNamePageProduct({
      ...namePageProduct,
      attributes_group_id:
        pageProduct[parent_key].data_child[index].attributes_group_id,
    });
  ...
  const changeShowIndex = (value, parent_index, index) => {
    let pageProduct = [...formik.values.product_page];
    let show_index =
      pageProduct[parent_index].data_child.length == 1 ? 1 : value;
  ...
  const callAPIInterPretProductPage = async (query) => {
	...
	//   console.log({query})
      ...
      interprets = await _productPageModel.getListInterPertProductPage(
        query.attributes_group_id
      );
    }

    product_page[query.index_parent].data_child[
      query.index_child
    ].data_interpret = interprets;
    formik.setFieldValue("product_page", product_page);
  };

  const setShowProductPage = async (
    data_interpret,
    index,
    parent_key,
    record = {}
  ...
  const handleDeleteChildProductPage = (child_key, parent_key) => {
    ...
    const index_parent = pageProduct.findIndex(
      (item, index) => index === parent_key
    );
    ...
    newPageProduct[index_parent].data_child.map((item, index) => {
      if (
        newPageProduct[index_parent].data_child[index_child].show_index != null
      ) {
        if (
      ...
      <TableAnt
        ...
        expandable={{
          expandedRowRender: (record, index) =>
            _expandableProductPage(index, record.data_child),
                  ...
                  <Col style={{ marginTop: 8 }} xs={12}>
                    {!noEdit && (
                      <Button
                        ...
                        disabled={
                          (formik.values.product_page || []).length ==
                          dataProductPage.length
                        }
                      >
                        <i className="fa fa-plus mr-2" />
                        Thêm dòng
