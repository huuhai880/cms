...
export default function ProductImage(props) {
   ...
   const fnUpload = (e) => {
      ...
      readFileAsBase64(image, {
         ...
         .then((imageBase64) => {
            const newImages = [
               ...images,
               ...imageBase64.map((e) => ({ ['product_id']: "", picture_url: e, is_default: false })),
            ];

            let checkDefault = newImages.filter(p => p.is_default);
            if (!checkDefault.length && newImages.length > 0) {
   ...
   const handleChangeDefaultPicture = (e) => {
      let newImages = [...images]
      newImages.forEach(el => {
         if(el.picture_url == e.picture_url){
            el.is_default = true
         }
         else{
            el.is_default = false
      ...
      <div ref={row} style={style ? { ...boxStyle, ...style } : boxStyle}>
         <Row>
            ...
            <Col xs={12}>
               <Row>
                  {images.length ? (
                     <Col xs={5} style={{ padding: "0px", position: "relative" }}>
                        <img
                           alt="image"
                           src={(images.find(p => p.is_default) || {}).picture_url}
                           className="image-product-default"
                           onClick={() => handleShowImage()}
                        />
                        {canEdit && (
                           <span
                              className="mr-1 btn-remove-img remove-image-default"
                              onClick={() => handleRemovePicture((images.find(p => p.is_default) || {}))}>
