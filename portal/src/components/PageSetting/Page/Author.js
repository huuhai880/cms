...
import Author5W1H from './Author5W1H';
// Model(s)
 ...
 * @class AuthorPage
 */
export default class AuthorPage extends PureComponent {
    ...
    formikValidationSchema = Yup.object().shape({
        AUTHOR_CAPTION: Yup.object().shape({
            value: Yup.string().required("Caption là bắt buộc.")
        }),
        AUTHOR_KOL_TITLE: Yup.object().shape({
            value: Yup.string().required("Tiêu đề 'Trợ lí viết sách cho các KOLs' là bắt buộc."),
        }),
        AUTHOR_KOL_DESCRIPTION: Yup.object().shape({
            value: Yup.string().required("Mô tả 'Trợ lí viết sách cho các KOLs là bắt buộc."),
        }),
        AUTHOR_THANKYOU_KOL_TITLE: Yup.object().shape({
            value: Yup.string().required("Tiêu đề 'Cảm ơn các KOLs đã tin tưởng SCC' là bắt buộc."),
        }),
        AUTHOR_THANKYOU_KOL_DESCRIPTION: Yup.object().shape({
            value: Yup.string().required("Mô tả 'Cảm ơn các KOLs đã tin tưởng SCC' là bắt buộc."),
        }),
        AUTHOR_5W1H_TITLE: Yup.object().shape({
            value: Yup.string().required("Tiêu đề '5W - 1H' là bắt buộc."),
        }),
        AUTHOR_5W1H_LIST: Yup.object().shape({
    ...
    getInitialValues() {
        let values = Object.assign(
            {}, {
            AUTHOR_CAPTION: {
                value: "Nơi tạo ra tác giả Hot",
                data_type: 'string'
            },
            AUTHOR_KOL_TITLE: {
                value: "Trợ lí viết sách cho các KOLs",
                data_type: 'string'
            },
            AUTHOR_KOL_DESCRIPTION: {
                value: "Dịch vụ Trợ lý viết sách giúp KOLs (Doanh nhân, diễn giả, nghệ sỹ…), những người có tầm ảnh hưởng muốn lưu lại di sản tri thức bằng cách Xuất bản những cuốn sách để đời.",
                data_type: 'string'
            },
            AUTHOR_THANKYOU_KOL_TITLE: {
                value: "Cảm ơn các KOLs đã tin tưởng SCC",
                data_type: 'string'
            },
            AUTHOR_THANKYOU_KOL_DESCRIPTION: {
    ...
    async _getBundleData() {
        let bundle = {};
        let all = [
            this._configModel.getPageConfig('AUTHOR')
    ...
    handleFormikSubmit(values, formProps) {
        ...
        let alerts = [];
        let apiCall = this._configModel.updatePageConfig('AUTHOR', values);
        apiCall
            .then(data => { // OK
    ...
    handleSort5W1H = (type, item) => {
        ...
        let { AUTHOR_5W1H_LIST: { value } } = values;
        let nextIdx = null;
        ...
        if ((foundIdx !== nextIdx) && (null !== nextIdx)) {
            ...
            handleChange({ target: { name: "AUTHOR_5W1H_LIST.value", value } });
        }
    }
    ...
    handleDelete(confirm, index) {
        ...
        if(confirm){
            ...
            handleChange({ target: { name: "AUTHOR_5W1H_LIST", value: {value: cloneData, data_type: 'json'} } });
        }
      }
                                    ...
                                    >
                                        <Row className="d-flex justify-content-center">
                                            ...
                                            <Col xs={12}>
                                                <Row className="mb-4">
                                                    <Col xs={8} className="mx-auto">
                                                        <b className="title_page_h1 text-primary underline">Trợ lí viết sách cho các KOLs</b>
                                                    </Col>
                                                </Row>
                                                <Row>
                                                    <Col xs={8} className="mx-auto">
                                                        <Row>
                                                            ...
                                                            <FormInput
                                                                ...
                                                                type="textarea"
                                                                name="AUTHOR_KOL_DESCRIPTION.value"
                                                                isEdit={!noEdit}
                                                                labelSm={3}
                                                                inputSm={9}
                                            ...
                                            <Col xs={12}>
                                                <Row className="mb-4">
                                                    <Col xs={8} className="mx-auto">
                                                        <b className="title_page_h1 text-primary underline">Cảm ơn các KOLs đã tin tưởng SCC</b>
                                                    </Col>
                                                </Row>
                                                <Row>
                                                    <Col xs={8} className="mx-auto">
                                                        <Row>
                                                            <FormInput
                                                                label="Tiêu đề"
                                                                name="AUTHOR_THANKYOU_KOL_TITLE.value"
                                                                labelSm={3}
                                                                inputSm={9}
                                                                isEdit={!noEdit}
                                                            />
                                                            <FormInput
                                                                label="Mô tả ngắn gọn"
                                                                type="textarea"
                                                                name="AUTHOR_THANKYOU_KOL_DESCRIPTION.value"
                                                                isEdit={!noEdit}
                                                                labelSm={3}
                                                                inputSm={9}
                                                                inputClassName="home-page_textarea"
                                                ...
                                                <Row>
                                                    <Col xs={8} className="mx-auto">
                                                        <Table bordered>
                                                            ...
                                                            <tbody>
                                                                {
                                                                    values.AUTHOR_5W1H_LIST &&
                                                                        values.AUTHOR_5W1H_LIST.value &&
                                                                        values.AUTHOR_5W1H_LIST.value.length ?
                                                                        values.AUTHOR_5W1H_LIST.value.map((item, index) => (
                                                                            <tr key={index}>
                                                                                <td style={{ width: 100 }}>
                                                                                    <Button
                                                                                        size="sm"
                                                                                    ...
                                                                                    <Button
                                                                                        size="sm"
                                                                                        color="success"
                                                                                        disabled={((values.AUTHOR_5W1H_LIST.value.length - 1) === index) || noEdit}
