...
class Service extends Component {
  ...
  async _getBundleData() {
    try {
      const [data] = await Promise.all([
        fnGet({ url: "service", query: this.state.query }),
      ]);
      return { data };
    } catch (err) {
  ...
  getData = (query = {}) => {
    this.setState({ isLoading: true });
    return fnGet({ url: "service", query }).then((res) => {
      let data = res.items;
      let isLoading = false;
  ...
  handleClickAdd = () => {
    window._$g.rdr("/service/add");
  };

  handleClose(confirm, id, rowIndex) {
    const { data } = this.state;
    if (confirm) {
      fnDelete({ url: `service/${id}` })
        .then(() => {
  ...
  render() {
    const columns = [
      configIDRowTable("service_id", "/service/detail/", this.state.query),
      {
        name: "service_name",
        label: "Tên dịch vụ",
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <RowAction
                ...
                editPermission="MD_SERVICE_EDIT"
                deletePermission="MD_SERVICE_DEL"
                // detailLink="/service/detail/"
                editLink="/service/edit/"
                deleteLink="/service/"
              />
            );
          },
          ...
          {this.state.toggleSearch && (
            <CardBody className="px-0 py-0">
              <div className="MuiPaper-filter__custom z-index-2">
                <ServiceFilter handleSubmit={this.handleSubmitFilter} />
              </div>
            </CardBody>
          )}
        </Card>
        <div>
          <CheckAccess permission="MD_SERVICE_ADD">
            <Button
          ...
          {/* <CheckAccess permission="MD_Service_EXPORT">
