...
function ParamType() {
  ...
  const [dataParamType, setDataParamType] = useState([]);
  ...
  const handleDelete = (id) => {
    window._$g.dialogs.prompt("Bạn có chắc chắn muốn xóa dữ liệu đang chọn?", "xóa", (confirm) => {
      if (confirm) {
        try {
          _paramTypeModel.delete(id).then((data) => {
          ...
          <Col xs={12} style={{ padding: 0 }}>
            <div className="MuiPaper-root__custom">
              {isLoading ? (
                ...
                <div>
                  <MUIDataTable
                    data={dataParamType.items}
                    columns={getColumTable(
                      dataParamType.items,
                      dataParamType.totalItems,
                      query,
                      handleDelete
                      // handleReply,
                      // handleReview
                    )}
                    options={configTableOptions(dataParamType.totalItems, query.page, {
                      itemsPerPage: query.itemsPerPage,
                    })}
                  />
                  <CustomPagination
                    count={dataParamType.totalItems}
