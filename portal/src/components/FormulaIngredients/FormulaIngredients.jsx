...
function FormulaIngredients() {
  ...
  const [dataFormulaIngredients, setDataFormulaIngredients] = useState([]);
  ...
  const _callAPI = async (props) => {
    try {
      ...
      //     setDataFormulaIngredients(data);
      //     // console.log(data);
      //   });
    } catch (error) {
      console.log(error);
    ...
    <div>
      ...
      <Col xs={12} sm={4} className="d-flex align-items-end mb-3" style={{ padding: 0 }}>
        <FormGroup className="mb-2 mb-sm-0">
          <CheckAccess permission={"FOR_FORMULAINGREDIENTS_ADD"}>
            <Button
              color="success"
          ...
          <Col xs={12} style={{ padding: 0 }}>
            <div className="MuiPaper-root__custom">
              {isLoading ? (
                ...
                <div>
                  {/* <MUIDataTable
                    data={dataFormulaIngredients.items}
                    columns={getColumTable(
                      dataFormulaIngredients.items,
                      dataFormulaIngredients.totalItems,
                      query,
                      handleDelete
                      // handleReply,
                      // handleReview
                    )}
                    options={configTableOptions(dataFormulaIngredients.totalItems, query.page, {
                      itemsPerPage: query.itemsPerPage,
                    })}
                  />
                  <CustomPagination
                    count={dataFormulaIngredients.totalItems}
