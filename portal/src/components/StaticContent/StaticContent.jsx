...
class StaticContent extends Component {
  ...
  state = {
    ...
    /** @var {Array} */
    StaticContentOptions: [{ name: "-- Chọn --", id: "" }],
  };
  ...
  componentDidMount() {
    ...
    (async () => {
      ...
      let { StaticContentOptions = [] } = this.state;
      StaticContentOptions = StaticContentOptions.concat(
        bundle.StaticContentOptions || []
      );
  ...
  getData = (query = {}) => {
    this.setState({ isLoading: true });
    return this._staticContentModel.list(query).then((res) => {
      let data = res.items;
      let isLoading = false;
