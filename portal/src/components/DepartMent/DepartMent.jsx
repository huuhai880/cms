...
/**
  ...
  getData = ( query = {} ) => {
    this.setState({ isLoading: true });
    return this._departmentModel.getList(query)
    .then(res => {
      let data = [...res.items];
      //let data = res.items || [];
      let isLoading = false;
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._departmentModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleDelete = (confirm, id, rowIndex) => {
    ...
    if (confirm) {
      this._departmentModel.delete(id)
      .then(() => {
  ...
  //Export excel
  handleExport = () => {
    this._departmentModel.exportExcel()
    .then(response => {
  ...
  render() {
    const columns = [
      configIDRowTable("department_id", "/department/detail/", this.state.query),
      {
                ...
                <CheckAccess permission="MD_DEPARTMENT_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].department_id, tableMeta['rowIndex'])
                    }}
                  />
                </CheckAccess>
              </div>
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="text-center">
                <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].department_id, tableMeta['rowIndex'])}>
                  <i className="fa fa-info" />
                </Button>
                <CheckAccess permission="MD_DEPARTMENT_EDIT">
                  <Button color="success" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].department_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="MD_DEPARTMENT_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].department_id, tableMeta['rowIndex'])}>
