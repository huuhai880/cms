...
export default class ContactCustomerAdd extends PureComponent {
  ...
  getInitialValues() {
    ...
    let values = Object.assign({}, this._contactCustomerModel.fillable());
  ...
  handleFormikSubmit(values, formProps) {
    ...
    this._contactCustomerModel
      .update(values.contact_id,formData)
      .then((data) => {
        window._$g.toastr.show("Lưu thành công!", "success");
        if (this._btnType === "save_n_close") {
          willRedirect = true;
          return window._$g.rdr("/contact");
        }
        return data;
      })
      .catch((apiData) => {
  ...
  render() {
    let { _id, ready, alerts, enableNote } = this.state;
        ...
        <Row>
          <Col xs={12}>
            <Card>
              <CardHeader>
                <b>
                  {contactCustomerEnt ? (noEdit ? "Chi tiết" : "Chỉnh sửa") : "Thêm mới"} liên hệ{" "}
                  {contactCustomerEnt ? contactCustomerEnt.full_name : ""}
                </b>
              </CardHeader>
              <CardBody>
                          ...
                          <Col xs={12}>
                                      ...
                                      <Col sm={8}>
                                        ...
                                        <ErrorMessage
                                          ...
                                          component={({ children }) => (
                                            <Alert
                                              color="danger"
                                              className="field-validation-errorr"
                                            >
                                              {children}
                                            </Alert>
                                          )}
                                          ...
                                          <Col sm={8}>
                                            <Field
                                              ...
                                              render={({ field /* _form */ }) => {
                                                ...
                                                // }
                                                let options = this.state.contactStatus.map(
                                                ...
                                                return (
                                                  <Select
                                                    // isDisabled={noEdit}
                                                    className="form-group MuiPaper-filter__custom--select"
                                                    // placeholder={}
                                                    onChange={(item) => {
                                                      this.setState({ enableNote: false });
                                                      field.onChange({
                                                        target: {
                                                          type: "select",
                                                          name: "contact_status",
                            ...
                            <Row>
                              <Col sm={12} className="text-right">
                                ...
                                <Button
                                  disabled={isSubmitting}
                                  onClick={() => window._$g.rdr("/contact")}
