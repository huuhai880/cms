...
export default class CompaniesAdd extends PureComponent {
  ...
  componentDidMount() {
    ...
    if (ID) {
      this._companyModel.read(ID).then((data) => {
  ...
  getInitialValues = () => {
    ...
    let values = Object.assign({}, this._companyModel.fillable(), companyData);
  ...
  async _getBundleData(companyData) {
    ...
    let all = [
      ...
      this._provinceModel
        .getOptions(
          (companyData && companyData.country_id) || CountryModel.ID_VN
        )
    ...
    if (companyData && companyData.province_id) {
      all = all.concat([
        this._districtModel
          .getOptions(companyData.province_id)
    ...
    if (companyData && companyData.district_id) {
      all = all.concat([
        this._wardModel
          .getOptions(companyData.district_id)
  ...
  formikValidationSchema = Yup.object().shape({
    ...
    address_full: Yup.string().required("Địa chỉ là bắt buộc."),
  });
  ...
  handleFormikSubmit = (values, formProps) => {
    ...
    // Build form data
    // +++
    let { open_date, address_full, is_active, company_name, email } = values;
                ...
                <b>
                  {companyData && companyData.company_name
                    ? noEdit
                      ...
                      : "Chỉnh sửa"
                    : "Thêm mới"}{" "}
                  công ty {companyData ? companyData.company_name : ""}
                </b>
              </CardHeader>
              <CardBody>
                {/* general alerts */}
                          ...
                          <Col xs={12}>
                            ...
                            <Row>
                              <Col sm={12} className="text-right">
                                {noEdit ? (
                                  <CheckAccess permission="AM_COMPANY_EDIT">
                                    <Button
                                      ...
                                      onClick={() =>
                                        window._$g.rdr(
                                          `/companies/edit/${companyData.company_id}`
                                        )
                                      }
                                    >
                                ...
                                <Button
                                  disabled={isSubmitting}
                                  onClick={() => window._$g.rdr("/companies")}
