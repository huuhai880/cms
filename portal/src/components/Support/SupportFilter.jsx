...
class SupportFilter extends PureComponent {
  constructor(props) {
    super(props)
    this.state = {
      inputValue: "",
  ...
  handleChangeSupportOptions = supportOptions => {
    this.setState({ supportOptions })
  }
  ...
  onSubmit = () => {
    const { inputValue, supportOptions, create_date_from, create_date_to, selectedIsActive } = this.state
    const { handleSubmit } = this.props
    handleSubmit(
      inputValue,
      supportOptions ? supportOptions.value : undefined,
  ...
  onClear = () => {
    const { inputValue, supportOptions, create_date_from, create_date_to, selectedIsActive } = this.state
    if (inputValue || supportOptions || create_date_from || create_date_to || selectedIsActive) {
      this.setState({
        inputValue: "",
        supportOptions: null,
  ...
  render() {
    const { supportOptions } = this.props;
          ...
          <Row>
            ...
            <FormGroup className="mb-2 mb-sm-0">
                ...
                <Input
                  ...
                  type="text"
                  name="inputValue"
                  placeholder="Nhập tên người liên hệ, số điện thoại , email"
            ...
            <Col xs={12} sm={3}>
              <FormGroup className="mb-2 mb-sm-0">
                <Label for="" className="mr-sm-2">Chủ đề</Label>
                <Col className="pl-0 pr-0">
                  <Select
                    className="MuiPaper-filter__custom--select"
                    id="supportOptions"
                    name="supportOptions"
                    onChange={this.handleChangeSupportOptions}
                    isSearchable={true}
                    placeholder={"-- Chọn --"}
                    value={this.state.supportOptions}
                    options={supportOptions.map(({ name: label, id: value }) => ({ value, label }))}
                  />
                </Col>
              </FormGroup>
            </Col>
