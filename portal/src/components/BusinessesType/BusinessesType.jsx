...
class BusinessesType extends Component {
  ...
  async _getBundleData() {
    ...
    let all = [
      // @TODO:
      this._BusinessTypeModel.getList(this.state.query)
  ...
  getData = ( query = {} ) => {
    this.setState({ isLoading: true });
    return this._BusinessTypeModel.getList(query)
    .then(res => {
      let data = res.items;
      let isLoading = false;
  ...
  handleClickAdd = () => {
    window._$g.rdr('/businesses-type/add')
  }
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._BusinessTypeModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleActionItemClick(type, id, rowIndex) {
    let routes = {
      detail: '/businesses-type/detail/',
      delete: '/businesses-type/delete/',
      edit: '/businesses-type/edit/',
      changePassword: '/businesses-type/change-password/',
    }
  ...
  handleClose(confirm, id, rowIndex) {
    ...
    if (confirm) {
      this._BusinessTypeModel.delete(id)
      .then(() => {
  ...
  render() {
    const columns = [
      configIDRowTable("business_type_id", "/businesses-type/detail/", this.state.query),
      {
                ...
                <CheckAccess permission="AM_BUSINESSTYPE_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].business_type_id, tableMeta['rowIndex'])
                    }}
                  />
                </CheckAccess>
              </div>
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="text-center">
                <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].business_type_id, tableMeta['rowIndex'])}>
                  <i className="fa fa-info" />
                </Button>
                <CheckAccess permission="AM_BUSINESSTYPE_EDIT">
                  <Button color="primary" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].business_type_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="AM_BUSINESSTYPE_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].business_type_id, tableMeta['rowIndex'])}>
    ...
    return (
      <div>
        ...
        <div>
          <CheckAccess permission="AM_BUSINESSTYPE_ADD">
          ...
          <CheckAccess permission="AM_BUSINESSTYPE_EXPORT">
