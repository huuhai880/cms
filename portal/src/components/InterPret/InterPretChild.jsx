...
function InterpretDetailChild() {
  let { id } = useParams();
  const _interpretModel = new InterpretModel();
  const [dataInterpretDetailDetail, setdataInterpretDetailDetail] = useState([]);
  ...
  const _callAPI = async (props) => {
    try {
      await _interpretModel.getListInterpretDetail(props).then((data) => {
        setdataInterpretDetailDetail(data);
        console.log(data);
      });
    } catch (error) {
      console.log(error);
        ...
        <FormGroup className="mb-2 mb-sm-0">
          <CheckAccess permission={"FOR_INTERPRET_DETAIL_ADD"}>
            <Button
              ...
              onClick={() => {
                window._$g.rdr(`/interpret/interpret-detail/add/${id}`);
              }}
              size="sm"
            >
          ...
          <Col xs={12} style={{ padding: 0 }}>
            <div className="MuiPaper-root__custom">
              {isLoading ? (
                ...
                <div>
                  <MUIDataTable
                    data={dataInterpretDetailDetail.items}
                    columns={getColumTable(
                      dataInterpretDetailDetail.items,
                      dataInterpretDetailDetail.totalItems,
                      query,
                      handleDelete
                      // handleReply,
                      // handleReview
                    )}
                    options={configTableOptions(dataInterpretDetailDetail.totalItems, query.page, {
                      itemsPerPage: query.itemsPerPage,
                    })}
                  />
                  <CustomPagination
                    count={dataInterpretDetailDetail.totalItems}
...
export default InterpretDetailChild;
