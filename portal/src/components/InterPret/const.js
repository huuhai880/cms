    ...
    return [
        ...
        },
        {
            ...
            options: {
                ...
                customBodyRender: (value, tableMeta, updateValue) => {
                    let { is_interpretspectial = false } = data[tableMeta["rowIndex"]] || {};
...
export const column = (handleDelete) => {
    return [
        ...
        {
            title: "Điều kiện",
            dataIndex: "is_condition_or",
            width: "6%",
            key: "is_condition_or",
            responsive: ["md"],
            render: (text, record, index) => (
                <div className="text-center">{record.is_interpretspectial ? (record.is_condition_or ? 'Hoặc' : 'Và') : '--'}</div>
            ),
        },
                            ...
                            <Button
                                ...
                                onClick={(evt) => {
                                    if (record.is_interpretspectial != 1) {
                                        ...
                                        window.open(
                                            `/portal/interpret/detail-web-spectial/${record["interpret_id"]}`,
                                            "_blank"
                                        );
                                    }
                                }}
