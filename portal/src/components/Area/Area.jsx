...
class Area extends Component {
  ...
  async _getBundleData() {
    ...
    let all = [
      // @TODO:
      this._areaModel.getList(this.state.query)
  ...
  getData = ( query = {} ) => {
    this.setState({ isLoading: true });
    return this._areaModel.getList(query)
    .then(res => {
      let data = res.items;
      let isLoading = false;
  ...
  onChangeStatus = (confirm, status, id, idx) => {
    if (confirm) {
      ...
      this._areaModel.changeStatus(id, postData)
      .then(() => {
  ...
  handleActionItemClick(type, id, rowIndex) {
    let routes = {
      ...
      delete: '/area/delete/',
      edit: '/area/edit/',
      changePassword: '/area/change-password/',
    }
  ...
  render() {
    const columns = [
      configIDRowTable("area_id", "/area/detail/", this.state.query),
      {
        name: "area_name",
        label: "Tên khu vực",
        options: {
                ...
                <CheckAccess permission="MD_AREA_EDIT">
                  <FormControlLabel
                    ...
                    onChange={event => {
                      ...
                      this.handleChangeStatus(checked, this.state.data[tableMeta['rowIndex']].area_id, tableMeta['rowIndex'])
                    }}
                  />
                </CheckAccess>
              </div>
      ...
      },
      {
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            return (
              <div className="text-center">
                <Button color="warning" title="Chi tiết" className="mr-1" onClick={evt => this.handleActionItemClick('detail', this.state.data[tableMeta['rowIndex']].area_id, tableMeta['rowIndex'])}>
                  <i className="fa fa-info" />
                </Button>
                <CheckAccess permission="MD_AREA_EDIT">
                  <Button color="primary" title="Chỉnh sửa" className="mr-1" onClick={evt => this.handleActionItemClick('edit', this.state.data[tableMeta['rowIndex']].area_id, tableMeta['rowIndex'])}>
                    <i className="fa fa-edit" />
                  </Button>
                </CheckAccess>
                <CheckAccess permission="MD_AREA_DEL">
                  <Button color="danger" title="Xóa" className="" onClick={evt => this.handleActionItemClick('delete', this.state.data[tableMeta['rowIndex']].area_id, tableMeta['rowIndex'])}>
    ...
    return (
      <div>
        ...
        <div>
          <CheckAccess permission="MD_AREA_ADD">
          ...
          <CheckAccess permission="MD_AREA_EXPORT">
