...
export default class NewsModel extends Model {
  ...
  /** @var {String} */
  static NEWS_NEWS_LAST_ITEM = "news/last-item";
  /** @var {String} */
  ...
  create(_data = {}) {
    ...
    data.small_thumbnail_image_file_id =
      data.small_thumbnail_image_file_id.toString().trim() == ""
        ? "0"
        : data.small_thumbnail_image_file_id.toString().trim();
    data.medium_thumbnail_image_file_id =
      data.medium_thumbnail_image_file_id.toString().trim() == ""
        ? "0"
        : data.medium_thumbnail_image_file_id.toString().trim();
    data.large_thumbnail_image_file_id =
      data.large_thumbnail_image_file_id.toString().trim() == ""
        ? "0"
        : data.large_thumbnail_image_file_id.toString().trim();
    data.xlarge_thumbnail_image_file_id =
      data.xlarge_thumbnail_image_file_id.toString().trim() == ""
        ? "0"
        : data.xlarge_thumbnail_image_file_id.toString().trim();
  ...
  changeStatus(id, _data) {
    ...
    return this._api.put(
      _static.NEWS_NEWS_CHANGE_STATUS.replace(":id", id),
      data
    );
  }
  /**
   ...
   *
   */
  getLastItem(id, _data = {}) {
    // Validate data?!
    let data = Object.assign({}, _data);
    return this._api.get(_static.NEWS_NEWS_LAST_ITEM, data);
  }
  ...
  deleteRelated(news_id, related_id) {
    return this._api.delete(
      _static.API_NEWS_DELETE_RELATED.replace(":news_id", news_id).replace(
        ":related_id",
        related_id
      )
    );
