...
export default class AccountModel extends Model {
      ...
      if (Object.keys(filters).length) {
        dataList = dataList.filter((item, idx) => {
          ...
          if (filters.groupname_or_description) {
            ...
            if (gnLC.indexOf(gnOrDscLC) >= 0 || dscLC.indexOf(gnOrDscLC) >= 0) {
              rtn = true;
            }
          }
          return rtn;
  ...
  GenCode(_data = {}) {
    ...
    let data = Object.assign({}, _data);
    //
    return this._api.get(_static.API_ACCOUNT_GEN_CODE, data);
  }
  /**
  ...
  getOptionsWonLost(_opts = {}) {
    ...
    return Promise.all([this.getOptions(optsIsWon), this.getOptions(optsIsLost)]).then(
      (dataArr) => {
  ...
  getOptionsFull(_opts) {
    ...
    return this.getList(apiOpts).then(({ items }) => {
      ...
      let ret = (items || []).map(({ user_group_id: id, user_group_name: name, description }) => {
